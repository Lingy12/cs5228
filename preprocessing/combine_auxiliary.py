...
def produce_mrt_features(df, mrt_df, threshold):
    df = df.copy()
    ...
    df_explode['mrt_dist'] = df_explode.progress_apply(lambda x: haversine(x['latitude_hdb'], x['longitude_hdb'], x['latitude_mrt'], x['longitude_mrt']), axis=1)
    ...
    # appending data
    df['near_mrt_count'] = near_mrt_count['near_count']
    df = pd.concat([df, nearest_mrt_dist], axis='columns')
    
    return df

def produce_school_features(df, school_df, threshold):
    df = df.copy()
    df = df.reset_index().rename(columns={"index": "list_id"})
    #find nearest mrt station, number of mrt within threshold t, distance to the nearest mrt
    df_explode = df.merge(school_df, how='cross', suffixes=('_hdb', '_school'))
    df_explode['school_dist'] = df_explode.progress_apply(lambda x: haversine(x['latitude_hdb'], x['longitude_hdb'], x['latitude_school'], x['longitude_school']), axis=1)
    ...
    # appending data
    df['near_school_count'] = near_mrt_count['near_count']
    df = pd.concat([df, nearest_school_dist], axis='columns')
    
    return df

def produce_mall_features(df, mall_df, threshold):
    df = df.copy()
    df = df.reset_index().rename(columns={"index": "list_id"})
    #find nearest mrt station, number of mrt within threshold t, distance to the nearest mrt
    df_explode = df.merge(mall_df, how='cross', suffixes=('_hdb', '_mall'))
    df_explode['mall_dist'] = df_explode.progress_apply(lambda x: haversine(x['latitude_hdb'], x['longitude_hdb'], x['latitude_mall'], x['longitude_mall']), axis=1)
    ...
    # appending data
    df['near_mall_count'] = near_mall_count['near_count']
